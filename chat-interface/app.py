...
def get_model_and_tokenizer():
    ...
    model_path = Path(".") / "gpt2-medium-355M-sft-it.pth"
    if not model_path.exists():
        print(
            f"Could not find the {model_path} file. Please run the instruction-follow"
            " notebook (instruction-follow.ipynb) to generate the gpt2-medium-355M-sft-it.pth file."
        )
        sys.exit()
